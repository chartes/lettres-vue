...
const actions = {
  ...
  async fetchAll({rootState, commit}) {
    ...
    try {
      ...
      const collections = collectionsJSON.map(({
        id,
        attributes: {title, description, legende_img, nb_docs, nb_pub_docs,date_min, date_max, date_min_pub, date_max_pub},
  ...
  fetchOne: async function({rootState, commit}, {id, numPage, pageSize, sortingPriority}) {
    ...
    const collection = {
      ...
      legende_img: c.attributes.legende_img,
  ...
  search ({ commit }, what) {
    ...
    http.get(`/search?query=*${what}*&index=collections&include=parents&without-relationships`).then( response => {
