<template>
  <section>
    ...
    <pagination
      ...
      :end="nbPages"
      :size="pageSize"
      :action="goToLocksPage"
      :bottom-widget="!compact"
    >
      <table
        class="table container is-narrow"
        :class="!compact ? 'is-bordered is-striped is-hoverable' : ''"
      >
        <thead>
          <tr>
            <!--
            ...
            <th>
              <input-filter
                label="Statut"
                place-holder="A ou E (Actif ou Expiré)"
                :action="filterStatus"
              />
            </th>
            <th v-if="!compact">
        ...
        >
          <tr>
            ...
            <td>
              <span
                v-if="lock.data.attributes['is-active']"
                class="tag is-warning status"
              >Actif</span>
              <span
                v-else
                class="tag is-static status"
              >Expiré</span>
            </td>
            <td v-if="!compact">
