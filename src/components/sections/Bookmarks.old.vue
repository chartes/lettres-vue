<template>
  <section v-if="current_user">
    ...
    <pagination
      v-if="userBookmarks && userBookmarks.length > 0"
      :current="currentPage"
      :end="nbPages"
      :size="pageSize"
      :action="goToBookmarkPage"
    >
      <table class="table is-narrow is-bordered is-striped is-hoverable container">
        ...
        <tbody>
          ...
          >
            ...
            <td v-html="bookmark.attributes['title']" />
            <td>
              <ul>
                <li
    ...
    <p v-else>
      <i>Vous n'avez aucun document mis en favoris.</i>
    </p>
  </section>
</template>
...
<script>
  ...
  export default {
    name: "Bookmarks",
    components : {Pagination, DocumentTagBar},
    props: {
